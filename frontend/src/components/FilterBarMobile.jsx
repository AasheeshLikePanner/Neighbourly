...
const FilterBar = ({ filters = [], setActiveFilter, activeFilter }) => {

    return (
        <div className="lg:hidden bg-white/80 backdrop-blur-xl rounded-2xl p-2 mb-6 border border-white/30 shadow-lg shadow-gray-200/20 overflow-x-auto">
            <div className="flex space-x-2 w-max">
                {filters.map(({ name, icon: Icon, count }) => (
                    <button
                        key={name}
                        onClick={() => setActiveFilter(name)}
                        className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 ${activeFilter === name
